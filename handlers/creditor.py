...
async def percent(message: Message, state: FSMContext):
    if message.text.lower() == "назад":
        ...
        if message.text.isdigit():
            ...
            result, p, prsum, Pv = await calc(data)
            ...
            await message.answer(f"```{str(result)}```", parse_mode=ParseMode.MARKDOWN)



            await message.answer_document(document=xls, caption="Для удобства отправляю этот же график в формате xlsx:")

            await message.answer(f"Сумма ежемесячного платежа: {round(p, 2)}\n"
                                 f"Итоговая сумма по кредиту: {round(Pv + prsum, 2)}")

            del p, Pv, prsum, xlsx, result, data
            await state.clear()
...
async def calc(data):
    ...
    prsum = 0
    df = pd.DataFrame(columns=['Ост.', '%', 'Осн. догл', 'Сумм плат.'])
    for i in range(n):
        procent = Pv * r
        Osn = P - procent
        Pv -= Osn
        prsum += round(procent, 2)
        df.loc[i] = [abs(round(Pv,2)), round(procent,2), round(Osn,2), round(P,2)]
    #return string
    df.index.name = '№'
    df.index = df.index + 1
    return df, P, prsum, float(data["amount"])
